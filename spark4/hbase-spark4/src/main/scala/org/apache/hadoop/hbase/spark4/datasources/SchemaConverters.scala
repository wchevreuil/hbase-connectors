...
object SchemaConverters {
  ...
  def toSqlType(avroSchema: Schema): SchemaType = {
    avroSchema.getType match {
      ...
      case UNION =>
        val typesSeq = avroSchema.getTypes.asScala.toSeq
        if (typesSeq.exists(_.getType == NULL)) {
          val remainingUnionTypes = typesSeq.filterNot(_.getType == NULL)
          if (remainingUnionTypes.size == 1) {
            toSqlType(remainingUnionTypes.head).copy(nullable = true)
          } else {
            val lst = new java.util.ArrayList[Schema]()
            remainingUnionTypes.foreach(s => lst.add(s))
            toSqlType(Schema.createUnion(lst)).copy(nullable = true)
          }
        } else {
          typesSeq.map(_.getType).toSeq match {
  ...
  def createConverterToSQL(schema: Schema): Any => Any = {
    schema.getType match {
      ...
      case RECORD =>
        val fieldConverters = schema.getFields.asScala.map(f => createConverterToSQL(f.schema)).toSeq
        val convertersSize = fieldConverters.size
        (item: Any) =>
          if (item == null) {
            null
          } else {
            val record = item.asInstanceOf[GenericRecord]
            val converted = new Array[Any](convertersSize)
            var idx = 0
            while (idx < convertersSize) {
      ...
      case ARRAY =>
        ...
        (item: Any) =>
          if (item == null) {
            ...
            try {
              item.asInstanceOf[java.util.List[Any]].asScala.map(elementConverter).toIndexedSeq
            } catch {
              case _: Throwable =>
                item.asInstanceOf[util.ArrayList[Any]].asScala.map(elementConverter).toIndexedSeq
            }
          }
      case MAP =>
        ...
        (item: Any) =>
          if (item == null) {
            ...
            item
              .asInstanceOf[java.util.HashMap[Any, Any]]
              .asScala
              .map { case (k, v) => (k.toString, valueConverter(v)) }
              .toMap
          }
      case UNION =>
        val typesSeq = schema.getTypes.asScala.toSeq
        if (typesSeq.exists(_.getType == NULL)) {
          val remainingUnionTypes = typesSeq.filterNot(_.getType == NULL)
          if (remainingUnionTypes.size == 1) {
            ...
            val lst = new java.util.ArrayList[Schema]()
            remainingUnionTypes.foreach(lst.add _)
            createConverterToSQL(Schema.createUnion(lst))
          }
        } else {
          typesSeq.map(_.getType).toSeq match {
            case Seq(t1, t2) if Set(t1, t2) == Set(INT, LONG) =>

...
/**
 * Directly ported from spark3 compatible module, this is intended for internal use in spark 4.
 * With DataSource V2, the entry point is always a TableProvider — that's the class Spark instantiates
 * when you call .format(...). It's the V2 equivalent of what DefaultSource (a RelationProvider) was in V1.
 *
 * The flow is:
 *  1. .format("org.apache.hadoop.hbase.spark.datasources.HBaseTableProvider") — Spark instantiates this class
 *  2. HBaseTableProvider reads the "catalog" option and parses it via HBaseTableCatalog(properties) internally
 *  3. It returns an HBaseTable, which returns an HBaseScanBuilder, and so on down the chain
 *
 ...
 *    val catalog = "..." // The actual json catalog string
 *    ...
 *    val df = spark.read.format("org.apache.hadoop.hbase.spark.datasources.HBaseTableProvider")
...
object HBaseTableCatalog {
      ...
      case (name, column) =>
        val sd = {
          column
            ...
            .map(n => Class.forName(n).getDeclaredConstructor().newInstance().asInstanceOf[SerDes])
        }
  ...
  //Directly ported from spark 3 compatible module, which was marking it as deprecated, but since this is
